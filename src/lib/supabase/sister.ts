...
//
// Mirror of contentpulse/lib/supabase/sister.ts. Used by the Stripe webhook
...
/**
 * Create a service-role client pointing at ContentPulse's Supabase
 ...
 *
 * Reads the new CONTENTPULSE_SUPABASE_* env vars first, then falls back
 * to the legacy RF_SUPABASE_* names so a rolling Vercel env rename does
 * not break suite mirroring during the cutover.
 */
export function createSisterClient(): SupabaseClient {
  const url = process.env.CONTENTPULSE_SUPABASE_URL ?? process.env.RF_SUPABASE_URL;
  const key =
    process.env.CONTENTPULSE_SUPABASE_SERVICE_ROLE_KEY ??
    process.env.RF_SUPABASE_SERVICE_ROLE_KEY;
  if (!url || !key) {
    throw new Error(
      'Sister-DB env vars missing (CONTENTPULSE_SUPABASE_URL + CONTENTPULSE_SUPABASE_SERVICE_ROLE_KEY, ' +
        'or legacy RF_SUPABASE_URL + RF_SUPABASE_SERVICE_ROLE_KEY). ' +
...
export const SISTER_PRODUCT_NAME = 'contentpulse';
