...
//
// COPY THIS FILE BETWEEN CP AND RF VERBATIM. Schema drift between
...
// ingester reject older versions.
...
export interface SignalPayload {
  ...
  /** RF's external client identifier (typically the RF client UUID). */
  rf_client_id: string;
  /** RF's display name for the client — CP uses this as the matcher
   *  fallback when no entry in cp_rf_client_map exists yet. */
  ...
  /** Email of the RF account that owns the client roster. CP uses this
   *  to scope the signal to the right agency. */
  agency_email: string;
  ...
  /** ISO 8601 — when RF actually computed the signal. */
  emitted_at: string;
}
