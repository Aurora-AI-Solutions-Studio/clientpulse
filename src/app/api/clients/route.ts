...
export async function GET(_request: NextRequest) {
  try {
    ...
    const mappedClients: Client[] = clients?.map((client: any) => ({
      ...
      healthScore: client.health_score || generateMockHealthScore(),
      status: client.status as string,
      lastMeetingDate: client.last_meeting_date as string | undefined,
  ...
  try {
    ...
    const { data: newClient, error: createError } = await supabase
      ...
      .insert({
        ...
        health_score: generateMockHealthScore(),
        status: 'active',
      })
      .select()
      .single();
    ...
    const mappedClient: Client = {
      ...
      healthScore: newClient.health_score,
      status: newClient.status,
      lastMeetingDate: newClient.last_meeting_date,
